  ...
  return (
    <div style={{ width: '100vw', height: '100dvh', background: '#000', display: 'flex', justifyContent: 'center', overflow: 'hidden' }}>
      <div style={{ position: 'relative', width: '100%', maxWidth: '1280px', height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ height: '85vh', position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 5 }}>
          ...
          {/* Play Mission Button */}
          <button type="button" onClick={() => handleNavigate('LEVEL_SELECT')} style={{ height: '35%', padding: 0, margin: '0 0 5vh 0', border: 'none', background: 'transparent', cursor: 'pointer', transition: 'transform 0.1s' }} onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'}>
          ...
          {/* Cards Row */}
          <div style={{ height: '40%', width: '90%', display: 'flex', justifyContent: 'center', gap: '2vw' }}>
...
const TopBar: React.FC<{ level: number; xp: number; xpTarget: number; xpProgress: number; gameCoins: number; cryptoPigs: number; username: string; onSettings: () => void; onBank: () => void; }> = ({ level, xp, xpTarget, xpProgress, gameCoins, cryptoPigs, username, onSettings, onBank }) => {
  return (
    <div style={{ height: '100%', display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(10,10,10,0.85)', backdropFilter: 'blur(4px)' }}>
      ...
      {/* GAME COINS (Off-Chain) */}
      <div style={{ flex: 0.8, ...topCellStyle, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <span style={{ fontSize: '9px', color: '#aaa', fontWeight: 'bold' }}>GAME COINS</span>
          <span style={{ fontSize: 'clamp(12px, 1.8vw, 16px)', fontWeight: 900, color: '#ffd700' }}>🪙 {gameCoins}</span>
        </div>
      </div>

      {/* $PIGS (On-Chain Crypto) */}
        ...
        <div style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              ...
              <span style={{ fontSize: 'clamp(12px, 1.8vw, 16px)', fontWeight: 900, color: '#fff1c9' }}>{cryptoPigs} $PIGS</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, ...iconCellStyle }}>
        <div style={{ transform: 'scale(0.8)', transformOrigin: 'center' }}><WalletButton /></div>
      </div>
...
const MenuCard: React.FC<{ src: string; alt: string; onClick: () => void }> = ({ src, alt, onClick }) => (
  <button type="button" onClick={onClick} style={{ height: '100%', padding: 0, border: 'none', background: 'transparent', cursor: 'pointer', display: 'flex', justifyContent: 'center', transition: 'transform 0.1s ease-in-out' }} onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'}>
