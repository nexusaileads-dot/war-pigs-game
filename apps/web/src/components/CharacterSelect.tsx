            ...
            return (
              <div key={cid} style={{ background: isSelected ? '#2a1b14' : '#222', padding:14, borderRadius:12, display:'flex', alignItems:'center', gap:14, border:`2px solid ${isSelected ? '#ff6b35' : '#333'}`, color:'#fff', boxShadow: isSelected ? '0 8px 20px rgba(255,107,53,0.18)' : 'none' }}>
                <button type="button" onClick={() => setSelectedCharacterId(cid)} style={thumbButtonStyle}>
                  <img src={getCharacterImage(cid)} alt={item.details.name} style={{maxWidth:'100%', maxHeight:'100%', objectFit:'contain'}} onError={e => e.currentTarget.src='/assets/sprites/Grunt-Bacon.png'} />
                </button>
