...
export const CharacterSelect: React.FC<{ onBack: () => void; onStart: () => void }> = ({ onBack, onStart }) => {
  ...
  // Load Inventory
  const loadInventory = useCallback(async () => {    try {
      setLoadError(null);
  ...
  const confirmSelection = async () => {
    ...
    try {
      ...
      // Proceed to next step (Weapon Select)
      onStart();
    } catch (error) {
  ...
  const upgradeCharacter = async (characterId: string) => {
    ...
    try {
      ...
      showNotification('Unit upgrade failed.', 'error');
    } finally {      setUpgradingCharacterId(null);
    }
  };

  // --- Render States ---
  // We keep the header visible even during loading to prevent layout shift
  const renderHeader = () => (
    <div style={headerRowStyle}>
      <button onClick={onBack} style={backButtonStyle}>BACK</button>
      <h2 style={titleStyle}>Units</h2>
  ...
  if (isLoading) {
    return (
      <div style={outerContainerStyle}>
        {renderHeader()}
        <div style={centerContainerStyle}>LOADING UNITS...</div>
      </div>
    );
  }
    ...
    return (
      <div style={outerContainerStyle}>
        ...
        <div style={cardStyle}>
          ...
          <p style={{color:'#bbb',margin:0}}>Your inventory has no character units yet. Buy one in the shop first.</p>
        </div>
      </div>
    );
  }
  ...
  // --- Main UI ---
  return (
    <div style={outerContainerStyle}>      {renderHeader()}
      ...
      <div style={scrollContainerStyle}>
        {selectedCharacter && selectedStats ? (
          <div style={selectionCardStyle}>
            <div style={imageBoxStyle}>
              <img src={getCharacterImage(selectedCharacter.details.characterId)} alt={selectedCharacter.details.name} style={{maxWidth:'100%', maxHeight:'100%', objectFit:'contain'}} onError={e => e.currentTarget.style.display='none'} />
            </div>
            ...
            <div style={imageBoxSmall}>
              <img src={getClassIcon(selectedCharacter.details.characterId, selectedCharacter.details.classType)} alt={selectedCharacter.details.classType} style={{width:38, height:38, objectFit:'contain'}} onError={e => e.currentTarget.src='/assets/sprites/assault.png'} />
            </div>
          </div>
        ) : null}
          ...
          {inventory.map(item => {
            ...
            return (
              <div key={cid} style={{ background: isSelected ? '#2a1b14' : '#222', padding:14, borderRadius:12, display:'flex', alignItems:'center', gap:14, border:`2px solid ${isSelected ? '#ff6b35' : '#333'}`, color:'#fff', boxShadow: isSelected ? '0 8px 20px rgba(255,107,53,0.18)' : 'none' }}>
                <button type="button" onClick={() => setSelectedCharacterId(cid)} style={thumbButtonStyle}>
                  <img src={getCharacterImage(cid)} alt={item.details.name} style={{maxWidth:'100%', maxHeight:'100%', objectFit:'contain'}} onError={e => e.currentTarget.src='/assets/sprites/Grunt-Bacon.png'} />                </button>
                <button type="button" onClick={() => setSelectedCharacterId(cid)} style={infoButtonStyle}>
                  ...
                  <div style={{display:'flex', flexWrap:'wrap', gap:8, marginBottom:6, color:'#888', fontWeight:'bold', fontSize:12}}>
        ...
        <button onClick={() => void confirmSelection()} disabled={!selectedCharacterId || isSubmitting} style={{ padding:'14px 20px', background:!selectedCharacterId||isSubmitting?'#555':'#ff6b35', border:'none', color:'#fff', borderRadius:10, cursor:!selectedCharacterId||isSubmitting?'not-allowed':'pointer', fontWeight:'bold', textTransform:'uppercase', letterSpacing:1, marginTop:'auto' }}>
          {isSubmitting ? 'CONFIRMING...' : 'CONFIRM UNIT'}
        </button>
      </div>
    </div>
  );
...
const imageBoxSmall: React.CSSProperties = { 
  background: '#101010', 
  padding: 10, 
  borderRadius: 10, 
  border: '1px solid #333', 
